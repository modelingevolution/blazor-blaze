export class SKHtmlCanvas {
    ...
    putImageData(pData, width, height) {
        ...
        // Note: ImageData constructor doesn't accept SharedArrayBuffer-backed arrays,
    ...
    static getGL() {
        return globalThis.SkiaSharpGL || SKHtmlCanvas.getModule().GL || GL;
    }
    static getModule() {
        if (globalThis.SkiaSharpModule) return globalThis.SkiaSharpModule;
        // .NET 10 with threading: Module is not a global, access via runtime
        if (typeof getDotnetRuntime === 'function') {
            const rt = getDotnetRuntime(0);
            if (rt?.Module) return rt.Module;
        }
        return Module;
    }
    static getGLctx() {
