...
# Functions
print_usage() {
    echo -e "${BLUE}BlazorBlaze Release Script${NC}"
    echo
    echo "Usage: ./release.sh [VERSION] [OPTIONS]"
    echo
    echo "Arguments:"
    echo "  VERSION     3-part version (e.g., 1.0.0, 1.2.3)"
    echo "              If not provided, script will auto-increment patch number"
    echo
    echo "Options:"
    ...
    echo "  ./release.sh 1.0.0 -m \"Initial release\"       # With release notes"
    echo "  ./release.sh --patch -m \"Bug fixes\"           # Auto-increment patch (default)"
    echo "  ./release.sh --minor -m \"New features\"        # Auto-increment minor"
...
get_latest_version() {
    git tag --sort=-version:refname | grep -E '^v[0-9]+\.[0-9]+\.[0-9]+$' | head -n1 | sed 's/^v//' || echo "0.0.0"
}
...
# Validate 3-part version
validate_version() {
    local version=$1
    if [[ ! $version =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
        print_error "Invalid version format: $version. Expected format: X.Y.Z (e.g., 1.0.0)"
        return 1
    fi
    return 0
}
...
create_release() {
    ...
    echo "3. Check package on NuGet.org: https://www.nuget.org/packages/BlazorBlaze"
...
main() {
    ...
    if [[ -z "$version" ]] && [[ "$increment_type" == "patch" ]] && [[ -z "$message" ]] && [[ "$dry_run" == "false" ]] && [[ "$auto_confirm" == "false" ]]; then
        ...
        echo "  ./release.sh --patch -y -m \"Bug fixes\"        # Auto-confirm release"
        exit 1
    fi
